...
The SVD of a matrix can be interpreted as a decomposition of a corresponding linear mapping $\Phi : \mathbb{R}^n \to \mathbb{R}^m$ into three operations.
Assume we are given a transformation matrix of a linear mapping $\Phi : \mathbb{R}^n \to \mathbb{R}^m$ with respect to the standard bases $B$ and $C$ of $\mathbb{R}^n$ and $\mathbb{R}^m$, respectively. 
Moreover, assume a second basis $\tilde{B}$ of $\mathbb{R}^n$ and $\tilde{C}$ of $\mathbb{R}^m$. Then:
\hfill \cite{mfml/book/mml/Deisenroth-Faisal-Ong}
