...
\textbf{Speech} & Speech $\to$ Text (ASR) & Whisper & CNN/Transformer & Transformer & \textbf{YES} \\ 
\hline

\textbf{Music} & Melody $\to$ Accompaniment & Music Transformer & Transformer & Transformer & \textbf{YES} \\ 
\hline

\textbf{Text (NLP)} & Autoencoding masked tokens & BERT & Transformer encoder & Transformer decoder (only for MLM head) & \\
\hline

\textbf{Multimodal Models} & Image $\to$ Caption & CLIP, Flamingo, BLIP & Vision encoder & Text decoder & Partly \\
\hline

\textbf{Generative Biology} & Protein sequence $\to$ structure & AlphaFold & Transformer encoder & Structure generator \\
\hline
