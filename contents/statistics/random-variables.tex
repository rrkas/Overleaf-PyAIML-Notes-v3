...
\begin{enumerate}
    ...
    .\hfill
    $
        N x y = \dsum^n_{i=1} 1_{\dCurlyBrac{x}}(X_i )\ 1_{\dCurlyBrac{y}}(Y_i )
    $
    \hfill
    where 
    $
    ...
    \item $\mbbV[\hat{κ}_C] \approx \dfrac{p_O (1 - p_O )}{n(1 - p_E )^2}$
    \hfill \cite{statistics/book/Statistics-for-Data-Scientists/Maurits-Kaptein}
...
\subsection{Comparison: Pearson’s Rho ($\rho_P$), Spearman’s Rho ($\rho_S$) or Kendall’s Tau Correlation ($\tau_K$)}
