...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \begin{enumerate}
            ...
            \item The specification of the prior can be tricky for two reasons: 
            \hfill \cite{mfml/book/mml/Deisenroth-Faisal-Ong}
            \begin{enumerate}
                \item the prior should encapsulate our knowledge about the problem before we see any data. This is often difficult to describe. 
                \hfill \cite{mfml/book/mml/Deisenroth-Faisal-Ong}
                
                \item it is often not possible to compute the posterior distribution analytically. 
                \hfill \cite{mfml/book/mml/Deisenroth-Faisal-Ong}
            \end{enumerate}
        \end{enumerate}
    ...
    \begin{definition}[Conjugate Prior]
        A prior is \textbf{conjugate} for the likelihood function if the posterior is of the same form/type as the prior.
        \hfill \cite{mfml/book/mml/Deisenroth-Faisal-Ong}
    \end{definition}
    \begin{enumerate}
        \item Conjugacy is particularly convenient because we can algebraically calculate our posterior distribution by updating the parameters of the prior distribution.
        \hfill \cite{mfml/book/mml/Deisenroth-Faisal-Ong}

        \item When considering the geometry of probability distributions, conjugate priors retain the same distance structure as the likelihood.
        \hfill \cite{mfml/book/mml/Deisenroth-Faisal-Ong}
    \end{enumerate}
...
\begin{enumerate}
    \item decision theory, when combined with probability theory, allows us to make optimal decisions in situations involving uncertainty such as those encountered in pattern recognition.
    \hfill \cite{ml/book/Pattern-Recognition-And-Machine-Learning/Christopher-M-Bishop}

    \item Suppose we have an input vector $\bm{x}$ together with a corresponding vector $\bm{t}$ of target variables, and our goal is to predict $\bm{t}$ given a new value for $\bm{x}$.
    \hfill \cite{ml/book/Pattern-Recognition-And-Machine-Learning/Christopher-M-Bishop}
    
    \item For regression problems, $\bm{t}$ will comprise continuous variables, whereas for classification problems $\bm{t}$ will represent class labels. 
    \hfill \cite{ml/book/Pattern-Recognition-And-Machine-Learning/Christopher-M-Bishop}

    \item The joint probability distribution $P(\bm{x}, \bm{t})$ provides a complete summary of the uncertainty associated with these variables.
    \hfill \cite{ml/book/Pattern-Recognition-And-Machine-Learning/Christopher-M-Bishop}

    \item we must often make a specific prediction for the value of $\bm{t}$, or more generally take a specific action based on our understanding of the values $\bm{t}$ is likely to take, and this aspect is the subject of decision theory.
    \hfill \cite{ml/book/Pattern-Recognition-And-Machine-Learning/Christopher-M-Bishop}
