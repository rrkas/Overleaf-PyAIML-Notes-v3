...
\begin{enumerate}
    ...
    \item the desiderata (meaning: something that is needed or wanted) for manipulating probability distributions in the machine learning context:
    \hfill \cite{mfml/book/mml/Deisenroth-Faisal-Ong}
    \begin{enumerate}
        \item There is some “closure property” when applying the rules of probability, e.g., Bayes’ theorem. 
        By closure, we mean that applying a particular operation returns an object of the same type.
        \hfill \cite{mfml/book/mml/Deisenroth-Faisal-Ong}

    \item  As we collect more data, we do not need more parameters to describe the distribution.
    \hfill \cite{mfml/book/mml/Deisenroth-Faisal-Ong}

    % \item  Since we are interested in learning from data, we want parameter estimation to behave nicely.
    % \hfill \cite{mfml/book/mml/Deisenroth-Faisal-Ong}
    \end{enumerate}

    \item There are three possible levels of \textbf{abstraction} we can have when considering distributions (of discrete or continuous random variables).
    \hfill \cite{mfml/book/mml/Deisenroth-Faisal-Ong}
    \begin{enumerate}
        \item At level one (the most concrete end of the spectrum), we have a particular \textbf{named distribution with fixed parameters}, for example a univariate Gaussian $\mathcal{N} (0, 1)$ with zero mean and unit variance.
        \hfill \cite{mfml/book/mml/Deisenroth-Faisal-Ong}

        \item In machine learning, we often use the second level of abstraction, that is, we fix the \textbf{parametric form} (the univariate Gaussian) and \textbf{infer the parameters from data}. 
        For example, we assume a univariate Gaussian $\mathcal{N} (\mu, \sigma^2)$ with unknown mean $\mu$ and unknown variance $\sigma^2$, and use a maximum likelihood fit to determine the best parameters $(\mu, \sigma^2)$.
        \hfill \cite{mfml/book/mml/Deisenroth-Faisal-Ong}

        \item A third level of abstraction is to consider \textbf{families of distributions}. 
        The univariate Gaussian is an example of a member of the exponential family. 
        \hfill \cite{mfml/book/mml/Deisenroth-Faisal-Ong}
    \end{enumerate}
\end{enumerate}
...
\begin{enumerate}
    \item statistic of a random variable is a deterministic function of that random variable
    \hfill \cite{mfml/book/mml/Deisenroth-Faisal-Ong}

    \item the idea that there are statistics that will contain all available information that can be inferred from data corresponding to the distribution under consideration.
    \hfill \cite{mfml/book/mml/Deisenroth-Faisal-Ong}

    \item sufficient statistics carry all the information needed to make inference about the population, that is, they are the statistics that are sufficient to represent the distribution.
    \hfill \cite{mfml/book/mml/Deisenroth-Faisal-Ong}
    \\
    As we observe more data, do we need more parameters $\theta$ to describe the distribution? 
    It turns out that the answer is yes in general.
    \hfill \cite{mfml/book/mml/Deisenroth-Faisal-Ong}

    \item For a set of distributions parametrized by $\theta $, let $X$ be a random variable with distribution $P(x | \theta 0)$ given an unknown $\theta _0$. 
    A vector $\phi(x)$ of statistics is called sufficient statistics for $\theta _0$ if they contain all possible information about $\theta _0$.
    To be more formal about “contain all possible information”, this means that the probability of $x$ given $\theta $ can be factored into a part that does not depend on $\theta $, and a part that depends on $\theta $ only via $\phi(x)$.
    \hfill \cite{mfml/book/mml/Deisenroth-Faisal-Ong}

    \item 
    \begin{theorem}[Fisher-Neyman (sufficient statistics)]
        Let $X$ have probability density function $P(x | \theta )$. 
        Then the statistics $\phi(x)$ are sufficient for $\theta $ if and only if $P(x | \theta )$ can be written in the form:
        \hfill \cite{mfml/book/mml/Deisenroth-Faisal-Ong}
        \\[0.2cm]
        .\hfill
    ...
    \end{theorem}
    \begin{enumerate}
        \item If $P(x | \theta )$ does not depend on $\theta $, then $\phi (x)$ is trivially a sufficient statistic for any function $\phi $. 
        \hfill \cite{mfml/book/mml/Deisenroth-Faisal-Ong}

        \item The more interesting case is that $P(x | \theta )$ is dependent only on $\phi (x)$ and not $x$ itself. 
        In this case, $\phi (x)$ is a sufficient statistic for $\theta $.
        \hfill \cite{mfml/book/mml/Deisenroth-Faisal-Ong}
    \end{enumerate}
\end{enumerate}
...
\begin{enumerate}[resume*=calcrulesrv]
    ...
    \item $\mbbV_X[x] = \mbbE_Y[ \mbbV_X[x|y] ] + \mbbV_Y[ \mbbE_X[x|y] ]$
    \hfill \cite{mfml/book/mml/Deisenroth-Faisal-Ong}
\end{enumerate}
\end{multicols}
...
\begin{enumerate}
    \item two approaches for obtaining distributions of transformations of random variables:
    \hfill \cite{mfml/book/mml/Deisenroth-Faisal-Ong}
    \begin{enumerate}
        \item a direct approach using the definition of a cumulative distribution function
        \hfill \cite{mfml/book/mml/Deisenroth-Faisal-Ong}

        \item change-of-variable approach that uses the chain rule of calculus
        \hfill \cite{mfml/book/mml/Deisenroth-Faisal-Ong}
    \end{enumerate}

    \item The change-of-variable approach is widely used because it provides a “recipe” for attempting to compute the resulting distribution due to a transformation.
    \hfill \cite{mfml/book/mml/Deisenroth-Faisal-Ong}

    \item Moment generating functions (MGFs) can also be used to study transformations of random variables
    \hfill \cite{mfml/book/mml/Deisenroth-Faisal-Ong}

    \item Suppose that there is a \textbf{discrete random variable} $X$ with pmf $P (X = x)$, and an invertible function $U (x)$. 
    Consider the \textbf{transformed random variable} $Y := U (X)$, with pmf $P (Y = y)$. 
    Then
    \hfill \cite{mfml/book/mml/Deisenroth-Faisal-Ong}
    \\[0.2cm]
    .\hfill
    ...
    \hfill \cite{mfml/book/mml/Deisenroth-Faisal-Ong}
    \\[0.2cm]
    where we can observe that $x = U ^{-1}(y)$.
    Therefore, for discrete random variables, transformations directly change the individual events (with the probabilities appropriately transformed).
    \hfill \cite{mfml/book/mml/Deisenroth-Faisal-Ong}

\end{enumerate}

\subsection{Distribution Function Technique}

\begin{enumerate}
    \item The distribution function technique goes back to first principles, and uses the definition of a cdf $F_X (x) = P (X \leq x)$ and the fact that its differential is the pdf $f (x) $.
    \hfill \cite{mfml/book/mml/Deisenroth-Faisal-Ong}

    \item For a random variable $X$ and a function $U$ , we find the pdf of the random variable $Y := U (X)$ by:
    \hfill \cite{mfml/book/mml/Deisenroth-Faisal-Ong}
    \begin{enumerate}
        \item Finding the cdf: $ F_Y (y) = P (Y \leq y) $
        \hfill \cite{mfml/book/mml/Deisenroth-Faisal-Ong}

        \item Differentiating the cdf $F_Y (y)$ to get the pdf $f (y)$: 
        $
            f(y) = \dfrac{d\ F_Y (y)}{d\ y} 
        $
        \hfill \cite{mfml/book/mml/Deisenroth-Faisal-Ong}
    \end{enumerate}
\end{enumerate}
